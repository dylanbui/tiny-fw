...
// 		$this->full_dst_path = $this->dest_folder.$filename.$this->thumb_marker.$file_ext;
	...
	{
		$image_config["source_image"] = $this->full_src_path;
		$image_config['create_thumb'] = FALSE;
		$image_config['maintain_ratio'] = TRUE;
		$image_config['new_image'] = $this->new_image;
		$image_config['quality'] = "100%";
		$image_config['width'] = $this->width;
		$image_config['height'] = $this->height;
	
		$dim = intval($this->orig_width) / intval($this->orig_height);
		if ($this->width != 0 && $this->height != 0)
			$dim = $dim - ($this->width / $this->height);
		...
		$resize_img_path = $this->full_src_path;
		if ($this->new_image !== "") {
			$resize_img_path = $this->new_image;
		}
	
		$image_config['source_image'] = $resize_img_path;
		$image_config['new_image'] = '';
		$image_config['quality'] = "100%";
		$image_config['maintain_ratio'] = FALSE;
		$image_config['x_axis'] = $this->x_axis;
		$image_config['y_axis'] = $this->y_axis;
	
		if ($this->x_axis == 0 && $this->y_axis == 0)
		{
			$resizeData = $this->get_image_properties($resize_img_path,TRUE);
			$image_config['x_axis'] = ($resizeData["width"] - $image_config['width']) / 2;
			$image_config['y_axis'] = ($resizeData["height"] - $image_config['height']) / 2;
		}
